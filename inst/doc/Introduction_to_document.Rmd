...
This is exactly what this package is about: run roxygen2 on (chunks of) single 
code files.

## Features
It enables you to
 - create function documentation with roxygen2 
 - save that documentation to disk
 - view that documentation in your interactive R session
It does this by creating a temporary package from (the chunks) of your single 
code file. So we can even use **R CMD check --as-cran** to see if your code and
documentation would pass a CRAN check 
(possibly detecting undocumented function arguments).
For the sake of cpu time on CRAN, I have disabled the package checks in all 
examples, you should always set check_package to ```TRUE```.
